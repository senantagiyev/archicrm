    ...
    <div class="overflow-hidden rounded-ds-xl border border-black/8 bg-card">
        <div class="flex flex-wrap items-center justify-between gap-6 bg-accent-dark px-6 py-7 text-white sm:px-8">
            <div class="min-w-0">
                ...
                <h1 class="mt-2 font-b2b text-heading font-semibold leading-tight sm:text-display">{{ $project->name }}</h1>
                <p class="mt-2 text-helper text-white/70">
            ...
            <div class="flex items-center gap-4">
                <div class="relative h-[74px] w-[74px]">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center text-body-lg font-bold">{{ $readiness }}%</div>
                </div>
                <div class="text-helper leading-snug text-white/70">
        ...
        <div class="grid grid-cols-2 divide-x divide-black/8 border-t border-black/8 sm:grid-cols-3">
            <div class="px-6 py-4">
                <p class="text-helper font-medium text-black/55">{{ t('portal.deadline') }}</p>
                <p class="mt-1 text-body font-semibold">{{ $project->deadline?->format('d.m.Y') ?? '—' }}</p>
            </div>
            <div class="px-6 py-4">
                <p class="text-helper font-medium text-black/55">{{ t('portal.manager') }}</p>
                <p class="mt-1 truncate text-body font-semibold">{{ $project->manager?->name ?? '—' }}</p>
            </div>
            <div class="col-span-2 border-t border-black/8 px-6 py-4 sm:col-span-1 sm:border-t-0">
                <p class="text-helper font-medium text-black/55">Cari mərhələ</p>
                <p class="mt-1 truncate text-body font-semibold">
    ...
    @if ($pendingApprovals > 0)
        <a href="{{ route('portal.approvals', $project) }}"
           class="mt-5 flex items-center gap-4 rounded-ds-xl border border-yellow-line bg-sel-bg px-6 py-4 transition-transform hover:-translate-y-0.5">
            ...
            <div class="flex-1">
                <p class="text-body font-semibold">{{ t('portal.pending_approvals', ['count' => $pendingApprovals]) }}</p>
                <p class="mt-0.5 text-helper text-black/60">Baxıb təsdiq və ya rədd etməyiniz gözlənilir.</p>
            </div>
            <span class="text-body-lg text-black/55">→</span>
        </a>
    @endif
        ...
        @foreach ($cards as [$key, $url, $label, $sub, $icon])
            <a href="{{ $url }}"
               class="group relative flex flex-col rounded-ds-xl border border-black/8 bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-black/20 hover:shadow-[0_10px_30px_-16px_rgba(0,0,0,.25)]">
                ...
                <p class="mt-4 text-body font-semibold">{{ $label }}</p>
                <p class="mt-1 text-helper text-black/55">{{ $sub }}</p>
            </a>
        @endforeach
    </div>
    ...
    <div class="overflow-hidden rounded-ds-xl border border-black/8 bg-card">
        @forelse ($project->stages as $stage)
            <div class="flex items-center gap-4 border-b border-black/5 px-5 py-4 last:border-0">
                <span @class([
                    'inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-helper font-semibold',
                    ...
                    'bg-neutral-soft text-black/55' => $stage->status === \App\Enums\StageStatus::NotStarted,
                ])>
                ...
                <span @class([
                    ...
                    'bg-error-soft text-error' => $stage->status === \App\Enums\StageStatus::Overdue,
                    'bg-neutral-soft text-black/60' => $stage->status === \App\Enums\StageStatus::NotStarted,
                ])>{{ $stage->status->translatedLabel() }}</span>
            </div>
        @empty
            <p class="px-5 py-10 text-center text-body text-black/55">{{ t('portal.no_stages') }}</p>
        @endforelse
    </div>
</x-portal.shell>
