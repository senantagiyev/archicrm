<x-portal.shell :title="t('portal.nav_documents')" :project="$project" active="documents">
    ...
    <h1 class="mb-5 text-2xl font-bold">{{ t('portal.nav_documents') }}</h1>

    @forelse ($documents as $document)
        <a href="{{ route('portal.documents.download', [$project, $document]) }}"
           class="group mb-3 flex items-center gap-4 rounded-[16px] border border-black/8 bg-white px-5 py-4 transition-all last:mb-0
            ...
            <span class="hidden shrink-0 rounded-pill bg-neutral-soft px-3 py-1 text-[12px] font-semibold text-black/55 sm:inline-block">
                {{ t('portal.download') }}
            </span>
    ...
    @empty
        <div class="rounded-[16px] border border-black/8 bg-white px-5 py-12 text-center">
            <p class="text-sm text-black/40">{{ t('portal.no_documents') }}</p>
        </div>
    @endforelse
</x-portal.shell>
