...
- **CAPTCHA**: Cloudflare Turnstile hər iki giriş formunda. `config/services.php` turnstile.{site_key,secret_key} (`.env` TURNSTILE_*). `TurnstileService::verify()` server tərəfli siteverify — açar YOXDURSA skip (dev/test), açar VARSA fail-closed. Portal: `Rules\Turnstile` validasiya qaydası. Filament: custom `App\Filament\Auth\Login` (token Livewire state-ə bağlanır, `authenticate()`-də yoxlanır).
