...
- **CAPTCHA**: Google reCAPTCHA v2 hər iki giriş formunda. `config/services.php` recaptcha.{site_key,secret_key} (`.env` RECAPTCHA_*). `RecaptchaService::verify()` server tərəfli siteverify (Google endpoint, field `g-recaptcha-response`) — açar YOXDURSA skip (dev/test), açar VARSA fail-closed. Portal: `Rules\Recaptcha` validasiya qaydası. Filament: custom `App\Filament\Auth\Login` (token Livewire state-ə bağlanır, `authenticate()`-də yoxlanır).
